...
- Última verificación local: `./mvnw.cmd clean verify` el **18 de septiembre de 2026**, con **244/244 pruebas sin fallos** (incluyendo `PortfolioIntegrationTest` 4/4, `OverdueInstallmentsJobIntegrationTest` 4/4, `PaymentPlanIntegrationTest` 6/6, `PaymentPlanRepositoryIntegrationTest` 5/5, `OpenApiDocsIntegrationTest` 1/1, y todas las suites previas de facturación, agenda, pacientes, CRM, autenticación y multi-tenancy). Este resultado no implica verificación en CI ni despliegue a producción.
- Fase siguiente: **Fase 7 — Especialistas externos e inventario** (siguiente ticket: **FASE7-01 — Modelar `Specialist` y liquidaciones**).
- Documentación interactiva: Swagger UI activo en dev/test (`/swagger-ui.html`, JSON en `/v3/api-docs`) con `springdoc-openapi` 3.1.1 y esquema `bearerAuth` JWT; deshabilitado en prod. Regla vigente (§4): todo endpoint nuevo se anota con `@Tag`/`@Operation` y cada cierre de fase verifica la UI. Cobertura de endpoints de cartera (`POST /api/v1/treatment-plans/{id}/payment-plan`, `POST /api/v1/installments/{id}/pay` y `GET /api/v1/portfolio/summary`) verificada en `OpenApiDocsIntegrationTest`.
