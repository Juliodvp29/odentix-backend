...
- Fase completada: **Fase 7 — Especialistas externos e inventario** (todos los tickets FASE7-01 a FASE7-04 completados; checklist de salida verificado y `ARCHITECTURE.md` actualizado).
- Última verificación local: `./mvnw.cmd clean verify` el **18 de septiembre de 2026**, con **273/273 pruebas sin fallos** (incluyendo `SpecialistRepositoryIntegrationTest` 7/7, `SettlementIntegrationTest` 7/7, `InventoryRepositoryIntegrationTest` 7/7, `InventoryIntegrationTest` 8/8, `OpenApiDocsIntegrationTest` 1/1 con los 8 endpoints nuevos, y todas las suites previas). Este resultado no implica verificación en CI ni despliegue a producción.
- Fase siguiente: **Fase 8 — Automatizaciones, notificaciones y tareas** (siguiente ticket: **FASE8-01 — Modelar `Task` y CRUD básico**).
- Documentación interactiva: Swagger UI activo en dev/test (`/swagger-ui.html`, JSON en `/v3/api-docs`) con `springdoc-openapi` 3.1.1 y esquema `bearerAuth` JWT; deshabilitado en prod. Regla vigente (§4): todo endpoint nuevo se anota con `@Tag`/`@Operation` y cada cierre de fase verifica la UI. Cobertura de endpoints de cartera (`POST /api/v1/treatment-plans/{id}/payment-plan`, `POST /api/v1/installments/{id}/pay` y `GET /api/v1/portfolio/summary`) verificada en `OpenApiDocsIntegrationTest`, más los 8 endpoints de Fase 7 (liquidación de especialista, CRUD/movimientos/críticos de inventario).
