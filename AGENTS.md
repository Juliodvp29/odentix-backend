...
- Última verificación local: `./mvnw.cmd clean verify` el **18 de septiembre de 2026**, con **225/225 pruebas sin fallos** (incluyendo `LeadIntegrationTest` 19/19, `OpenApiDocsIntegrationTest` 1/1, `LeadRepositoryIntegrationTest` 5/5, `EssentialPlanFlowIntegrationTest` 1/1, `InvoiceIntegrationTest` 9/9, `BillingModelIntegrationTest` 4/4, `TreatmentPlanIntegrationTest` 10/10, `TreatmentPlanRepositoryIntegrationTest` 7/7, y todas las suites previas de agenda, pacientes, autenticación y multi-tenancy). Este resultado no implica verificación en CI ni despliegue a producción.
- Fase en progreso: **Fase 6 — Cartera y pagos por etapas** (siguiente ticket: **FASE6-02 — Endpoints de planes de pago**).
- Última verificación local: `./mvnw.cmd test -Dtest=PaymentPlanRepositoryIntegrationTest` el **18 de septiembre de 2026**, con **5/5 pruebas sin fallos** (`PaymentPlanRepositoryIntegrationTest`: cuotas en estado pendiente, búsqueda por tratamiento, constraint UNIQUE, aislamiento cross-tenant en PaymentPlan e Installment). Suite global pendiente de verificación para este ticket.
