...
- Última verificación local: `./mvnw.cmd clean verify` el **18 de septiembre de 2026**, con **240/240 pruebas sin fallos** (incluyendo `OverdueInstallmentsJobIntegrationTest` 4/4, `PaymentPlanIntegrationTest` 6/6, `PaymentPlanRepositoryIntegrationTest` 5/5, `OpenApiDocsIntegrationTest` 1/1, y todas las suites previas de facturación, agenda, pacientes, autenticación y multi-tenancy). Este resultado no implica verificación en CI ni despliegue a producción.
- Fase en progreso: **Fase 6 — Cartera y pagos por etapas** (completados: FASE6-01, FASE6-02 y FASE6-03; siguiente ticket: **FASE6-04 — Dashboard de cartera**).
- Documentación interactiva: Swagger UI activo en dev/test (`/swagger-ui.html`, JSON en `/v3/api-docs`) con `springdoc-openapi` 3.1.1 y esquema `bearerAuth` JWT; deshabilitado en prod. Regla vigente (§4): todo endpoint nuevo se anota con `@Tag`/`@Operation` y cada cierre de fase verifica la UI. Cobertura de endpoints de cartera (`POST /api/v1/treatment-plans/{id}/payment-plan` y `POST /api/v1/installments/{id}/pay`) verificada en `OpenApiDocsIntegrationTest`.
