...
- Fase en progreso: **Fase 6 — Cartera y pagos por etapas** (siguiente ticket: **FASE6-03 — Job de cuotas vencidas**).
- Última verificación local: `./mvnw.cmd test -Dtest=PaymentPlanIntegrationTest` el **18 de septiembre de 2026**, con **6/6 pruebas sin fallos** (`PaymentPlanIntegrationTest`: crear plan con N cuotas, pagar cuota genera Invoice+Payment, 409 en segundo pago y segundo plan, aislamiento cross-tenant). Suite global pendiente de verificación para este ticket.
