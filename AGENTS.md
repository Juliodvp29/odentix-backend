...
- Fase actual: **Fase 2 — Pacientes e historia clínica base**, en curso. Tickets **FASE2-01 a FASE2-09 completados**: entidad `Patient` con aislamiento por tenant, CRUD con DTOs y baja lógica, paginación y búsqueda sin distinguir mayúsculas/acentos, manejo global de errores con `ApiErrorResponse`, pruebas cross-tenant, modelo y endpoints de `ClinicalRecord`, modelo y endpoints de `OdontogramEntry`, e integración de almacenamiento S3-compatible con metadatos en `PatientFile`, subida multipart y compensación de fallos contra huérfanos.
- Última verificación local: `./mvnw.cmd clean verify` el **16 de septiembre de 2026**, con **94/94 pruebas sin fallos**, incluidas **7/7 de `PatientFile*`** (`PatientFileUploadIntegrationTest` y `PatientFileRepositoryIntegrationTest`). Este resultado no implica verificación del último cambio en CI ni despliegue a producción.
- Siguiente ticket: **FASE2-10 — Endpoint de descarga y tests** (listar archivos de paciente y URL de descarga / descarga de archivo con pruebas cross-tenant), cerrando el checklist de salida de Fase 2.
- La Fase 2 todavía no está cerrada: faltan los tickets restantes, su checklist de salida y la actualización de `ARCHITECTURE.md` al cierre de la fase.
